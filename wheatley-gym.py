...
total_n = 200 # Number of total neurons
audio_n = 0 # Number of audio neurons
base_n = 2 # Number of generated input dimensions (e.g. xor has 2)
...
pixels, channels = 10, 1  # Dimensions of visual input
...
gamma = 0.002 # How strongly to update at every learning step
...
decay = 1 - gamma / 100 # How much connections decay every time-step

# Should video be used?
video_stream = False
exp_decay = np.power(gamma, -np.arange(firing_history - 1))[None, :, None]


class Mind:
    def __init__(self, threader, init_gamma, mode=None, long_plasticity=False):
...
def main():
    ...
    def processing(count, env=None):
        ...
        if wheatley.mode == "gym":
            if wheatley.firings[-1][-output_n:].mean() >= wheatley.firings[:, -output_n:].mean():
                action = 0
            else:
                action = 1
    ...
    for cur in range(counts):
        ...
        wheatley = Mind(threader, gamma, mode=mode)
